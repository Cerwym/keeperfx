...
struct AchievementBackend {
    const char* name;
    enum AchievementPlatform platform_type;
    ...
    TbBool (*unlock)(const char* achievement_id);
    TbBool (*is_unlocked)(const char* achievement_id);
    void (*set_progress)(const char* achievement_id, float progress);
    float (*get_progress)(const char* achievement_id);
